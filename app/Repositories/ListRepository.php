...
class ListRepository implements BaseRepository
{
    ...
    // this is just a copy of listFilteredQuery this.. small change is in else .id to .parent_id
    public function listFilteredQueryCategory($filter_columns)
    {
        $list = $this->model;
        if (isset($_GET['sortCol'])) {
            $list = $list->orderBy($_GET['sortCol'], ($_GET['sortByDesc'] == 1 ? 'desc' : 'asc'));
        } else {
            $list = $list->orderBy($list->getTable() . '.id', 'asc');
        }
        if (!empty($_GET['search'])) {
            $list = $list->Where(
                function ($query) use ($filter_columns) {
