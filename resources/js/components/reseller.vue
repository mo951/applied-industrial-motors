<template>
    <div>
      ...
      >
        ...
        <template v-slot:item.type="{ item }">
            <v-chip v-if="item.type==0" class="ma-2" color="secondary">Reseller</v-chip>
            <v-chip v-if="item.type==1" class="ma-2" color="teal">Agent</v-chip>
        </template>
      </v-data-table>
    </div>
  </template>
  ...
  const resellerservice = new service('resellers')
  export default {
    data() {
      return {
        search: "",
